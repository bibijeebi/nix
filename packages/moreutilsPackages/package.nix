...
let
  # List of all binaries in moreutils
  binaries = [
    "chronic"
    "combine"
    "errno"
  ...
  mkMoreUtilsPackages =
    moreutils:
    let
      packages = lib.genAttrs binaries (name: mkMoreUtilsPackage name);
    in
    packages
    // {
      ...
      overrideDerivation = f: mkMoreUtilsPackages (moreutils.overrideDerivation f);
    };

in
lib.recurseIntoAttrs (mkMoreUtilsPackages moreutils)
